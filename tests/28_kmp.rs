// Same as ./28_brute-force.rs but implemented using the Knuth Morris Pratt algorithm.
// Using KMP we are able to reduce the time complexity from O(h*n) to O(h + n) where
// h is the haystack size, and n is the needle size.
